...
export class CubicBezier extends Shape {
    ...
    constructor(
        ...
        x1: number, y1: number,  // управляющая 1
        x2: number, y2: number,  // управляющая 2
        x3: number, y3: number   // конец
    ) {
    ...
    flattenLocal(segments: number = 50): { x: number; y: number }[] {
    ...
    getDevicePoints(segments: number = 50): { x: number; y: number }[] {
    ...
    getControlPoints(): { x: number; y: number }[] {
        ...
        return [
            ...
            { x: this._p3.x + cx, y: this._p3.y + cy }
        ];
    }
    ...
    setControlPoint(idx: number, x: number, y: number): void {
        ...
        else if (idx === 3) this._p3 = { x: localX, y: localY };
    }
    ...
    toJSON(): object {
        ...
        return {
            ...
            x3: this._p3.x + cx, y3: this._p3.y + cy,
